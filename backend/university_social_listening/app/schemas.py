...
class UserInviteCreate(BaseModel):
    ...
    expiration_days: Optional[int] = Field(default=7, ge=1, le=90, description="Invite validity duration in days")

    model_config = {
        "json_schema_extra": {
            "example": {
                "email": "staff.somchai@up.ac.th",
                "role": "staff",
                "category_id": 1,
                "expiration_days": 7
            }
...
class UserInviteExtend(BaseModel):
    days: int = Field(default=7, ge=1, le=90, description="Days to extend the invitation by")
...
class UserInviteResend(BaseModel):
    expiration_days: Optional[int] = Field(default=7, ge=1, le=90, description="Days the renewed token will be valid for")
