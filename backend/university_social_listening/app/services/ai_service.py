...
# ── MULTI-LABEL CATEGORY KEYWORD & INTENT TAXONOMY ──────────────
CATEGORY_INTENT_MAP = {
    ...
    2: { # ระบบเครือข่ายและเทคโนโลยี
        ...
        "high_priority": ["ไวไฟเข้าไม่ได้", "wifi เข้าไม่ได้", "เน็ตล่ม", "ระบบล่ม", "ลงทะเบียนไม่ได้", "เว็บ reg ล่ม", "เข้าเว็บไม่ได้", "ลืมรหัสผ่าน", "อินเทอร์เน็ตหลุด"],
        "keywords": ["เน็ต", "wifi", "ไวไฟ", "wi-fi", "อินเทอร์เน็ต", "เน็ตช้า", "เน็ตหลุด", "หลุดบ่อย", "reg", "web", "เว็บไซต์", "ระบบ", "เข้าไม่ได้", "เซิร์ฟเวอร์", "เชื่อมต่อไม่ได้", "รหัสผ่าน", "up connect"],
    },
    3: { # การเรียนการสอนและวิชาการ
        "name": "การเรียนการสอนและวิชาการ",
        "high_priority": ["ตารางสอบชน", "ตารางเรียนชน", "ขอสอบชดเชย", "ขอเปิดเซค", "ดรอปเรียน", "ระบบเกรด", "ยื่นคำร้องวิชาการ"],
        "keywords": ["ตารางเรียน", "ตารางสอบ", "วันสอบ", "เกรด", "หน่วยกิต", "วิชา", "ลงทะเบียนเรียน", "อาจารย์", "ผู้สอน", "ห้องสอบ", "ขอสอบ", "ดรอป", "ใบเกรด", "วิชาการ", "การสอน"],
    },
    4: { # ภูมิทัศน์และความสะอาด
        "name": "ภูมิทัศน์และความสะอาด",
        "high_priority": ["หมาตาย", "สัตว์ตาย", "ซากสัตว์", "เลือดสาด", "ขยะล้น", "ถังขยะแตก", "ต้นไม้ล้มขวาง", "กิ่งไม้หักทับ", "ห้องน้ำสกปรกมาก", "กลิ่นเหม็นเน่า", "หนอนขึ้น"],
        "keywords": ["สกปรก", "ขยะ", "ถังขยะ", "เหม็น", "กลิ่นเหม็น", "ไม่ทำความสะอาด", "แม่บ้าน", "หญ้ารก", "ต้นไม้", "กิ่งไม้", "ใบไม้ร่วง", "ซาก", "คราบ", "เปื้อน", "เชื้อรา", "หนู", "แมลงสาบ", "ทำความสะอาด", "ภูมิทัศน์", "สิ่งแวดล้อม"],
    },
    5: { # ความปลอดภัยและจราจร
        "name": "ความปลอดภัยและจราจร",
        "high_priority": ["รถชน", "รถล้ม", "ชนหมา", "ชนคน", "รถเฉี่ยว", "ขโมย", "หมวกกันน็อกหาย", "ของหาย", "คนแปลกหน้า", "งูเข้า", "ไฟไหม้", "ควันไฟ", "อันตรายมาก", "มีผู้บาดเจ็บ"],
        "keywords": ["ชน", "อุบัติเหตุ", "เฉี่ยว", "จราจร", "ทางมืด", "ไฟกิ่งดับ", "เปลี่ยว", "รปภ", "รปภ.", "กล้องวงจรปิด", "ลักทรัพย์", "ขโมย", "หาย", "งู", "ตัวเงินตัวทอง", "สัตว์มีพิษ", "อันตราย", "ความปลอดภัย", "บาดเจ็บ", "ล้ม"],
    },
    6: { # บริการทั่วไป / อื่นๆ
        "name": "บริการทั่วไป / อื่นๆ",
        "high_priority": ["ทุนการศึกษา", "บัตรนิสิตหาย", "สวัสดิการนิสิต", "ร้องเรียนทั่วไป"],
        "keywords": ["ทุน", "ทุนการศึกษา", "กิจกรรม", "บัตรนิสิต", "สอบถาม", "ขอคำแนะนำ", "ทั่วไป", "อื่นๆ", "เสียงดัง", "รบกวน", "ประชาสัมพันธ์", "หอพัก"],
    },
    ...
    8: { # สุขอนามัย/ความปลอดภัยทางอาหาร
        ...
        "high_priority": ["อาหารเป็นพิษ", "พบสิ่งแปลกปลอมในอาหาร", "พบแมลงสาบในอาหาร", "ข้าวบูด", "อาหารไม่สุก", "ท้องเสียจากอาหาร"],
        "keywords": ["อาหาร", "โรงอาหาร", "กับข้าว", "จานชาม", "ช้อนส้อม", "น้ำดื่ม", "สุขอนามัย", "ความสะอาดอาหาร", "ร้านค้า", "ร้านอาหาร", "แมลงวัน", "เศษผม", "บูด", "ท้องเสีย"],
    }
}


def classify_problem_multilabel(
    text: str,
    categories_list: Optional[List[Dict]] = None,
    threshold: Optional[float] = None
) -> Dict[str, Any]:
    """
    Multi-label Classification Engine for UP Connect.
    Computes confidence probability scores across all active categories.
    Applies configurable Super Admin threshold to auto-route across multiple departments.
    """
    if not text:
        return {
            ...
            "top_confidence": 0.50,
            "threshold_used": threshold or 0.70,
            "routed_categories": [],
            "all_scores": []
        }

    # Dynamically fetch active categories from DB if not passed
    if not categories_list:
        try:
            from app.database import SessionLocal
            from app.models import Category
            db = SessionLocal()
            db_cats = db.query(Category).filter(Category.is_active == True).all()
            categories_list = [
                {"category_id": c.category_id, "category_name": c.category_name, "description": c.description or ""}
                for c in db_cats
            ]
            db.close()
        except Exception as e:
            logger.warning(f"Error loading categories from DB: {e}")
            categories_list = []

    # Fetch configured threshold from DB if not explicitly passed
    if threshold is None:
        try:
            ...
            if setting and setting.confidence_threshold is not None:
                threshold = float(setting.confidence_threshold)
            else:
                threshold = 0.50
            db.close()
        except Exception:
    ...
    # 1. Primary Engine: Model 2 WangchanBERTa Contextual Multi-label Classification
    try:
        from app.services.transformer_service import classify_with_wangchanberta
        wb_result = classify_with_wangchanberta(text, categories_list, threshold=threshold)
        if wb_result and wb_result.get("all_scores"):
            return wb_result
    except Exception as e:
        logger.warning(f"WangchanBERTa inference fallback: {e}")

    # 2. Fallback Dynamic Rule Engine
    ...
    for c in categories_list:
        ...
        # Check description & name keywords
        for w in re.split(r'[\s,./\-\(\)]+', cname + " " + cdesc):
            w = w.strip().lower()
        ...
        if match_score == 0:
            ...
            prob = min(0.98, (match_score / (match_score + 1.2)) * 0.95 + 0.15)
        ...
        scores_dict[cid] = {
            ...
            "raw_match": round(match_score, 2)
        }

    sorted_scores = sorted(scores_dict.values(), key=lambda x: x["confidence"], reverse=True)
    top = sorted_scores[0]
    if top["confidence"] < 0.30:
        primary_id = 6
        primary_name = "บริการทั่วไป / อื่นๆ"
        top_conf = 0.50
    else:
        primary_id = top["category_id"]
...
def suggest_category(text: str, categories_list: List[Dict]) -> int:
    """
    Predicts the primary category_id using the Multi-label Engine.
    """
    result = classify_problem_multilabel(text, categories_list)
    pred_id = result["primary_category_id"]
    
    # Validate against active categories_list if provided
    if categories_list and any(c.get("id") == pred_id or c.get("category_id") == pred_id for c in categories_list):
        return pred_id
...
def find_similar_problems(description: str, all_active_problems: List[Dict], category_id: int = None) -> List[Dict]:
    """
    Finds similar problems using Model 3: Sentence-BERT (SBERT) Semantic Vectors & Cosine Similarity.
    Automatically clusters duplicates for Zero-Click Ticket Aggregation.
    Falls back to Token-based Jaccard overlap if SBERT is offline.
    """
    if not description or not all_active_problems:
        return []

    # 1. Attempt SBERT Dense Vector Semantic Search
    try:
        import numpy as np
        from app.services.transformer_service import compute_post_embeddings
        all_descriptions = [p.get("description", "") or p.get("title", "") for p in all_active_problems]
        all_texts = [description] + all_descriptions
        embeddings = compute_post_embeddings(all_texts)
        
        if embeddings is not None and len(embeddings) > 1:
            target_emb = embeddings[0]
            other_embs = embeddings[1:]
            scored_problems = []
            for idx, p in enumerate(all_active_problems):
                sim = float(np.dot(target_emb, other_embs[idx]))
                # Strict SBERT cosine similarity threshold (>= 0.70) for semantic duplicates
                if sim >= 0.70:
                    scored_problems.append({"problem": p, "score": sim})
            
            if scored_problems:
                scored_problems.sort(key=lambda x: x["score"], reverse=True)
                return [item["problem"] for item in scored_problems[:5]]
    except Exception as e:
        logger.warning(f"SBERT similarity evaluation skipped/failed: {e}")

    # 2. Fast Token-based Jaccard Overlap Fallback
    import re
    def tokenize(text):
    ...
    for p in all_active_problems:
        ...
        if similarity > threshold:
            scored_problems.append({"problem": p, "score": similarity})
