...
@router.post("/test-routing", response_model=StandardResponse)
def test_auto_routing(
    ...
    cats = db.query(Category).filter(Category.is_active == True).all()
    cats_list = [{"id": c.category_id, "name": c.category_name} for c in cats]
    
    result = classify_with_wangchanberta(text, categories_list=cats_list, threshold=threshold)
    
    return StandardResponse(
        success=True,
        message="Multi-label auto-routing simulated successfully",
        data=result
    )
