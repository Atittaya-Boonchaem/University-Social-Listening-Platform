...
from app.tags import OPENAPI_TAGS, TAG_MASTER_DATA
...
# ──────────────────────────────────────────────
# Security: WAF & IDS Firewall Middleware
# ──────────────────────────────────────────────
from app.middleware.waf_middleware import WAFSecurityMiddleware
...
app.include_router(problems.router,          prefix="/api/v1/problems")
...
app.include_router(public_user_types.router, prefix="/api/v1")
app.include_router(clusters.router,          prefix="/api/v1/clusters")

# ──────────────────────────────────────────────
# Health check
# ──────────────────────────────────────────────
@app.get("/health", tags=[TAG_MASTER_DATA])
...
def seed_initial_data():
    ...
    try:
        ...
        all_up_buildings = [
            ("คณะเทคโนโลยีสารสนเทศและการสื่อสาร (ICT)", 19.027329, 99.8999566),
