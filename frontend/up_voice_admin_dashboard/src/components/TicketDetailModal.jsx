...
import { X, Calendar, MapPin, User, FileText, Image as ImageIcon, Forward, Shield, ShieldOff, Layers, CheckCircle2, AlertCircle, Clock, Sparkles, Tag, BrainCircuit } from 'lucide-react';
...
export default function TicketDetailModal({ ticket, onClose, onStatusChange, onForward, onQuarantine, onMerge }) {
  ...
  const authorRole = ticket.author?.role || 'นิสิต มพ.';
  ...
  const modalContent = (
    ...
    >
      ...
      {selectedImage && (
        <div 
          className="fixed inset-0 bg-black/85 z-[100000] flex items-center justify-center p-4 cursor-pointer"
        ...
        >
          <div className="relative max-w-4xl max-h-[90vh]" onClick={(e) => e.stopPropagation()}>
            ...
            <button 
              onClick={() => setSelectedImage(null)}
              className="absolute -top-4 -right-4 w-10 h-10 bg-white rounded-full text-slate-800 font-bold shadow-lg flex items-center justify-center hover:bg-slate-100"
            >
              ✕
            </button>
          </div>
      ...
      {/* Main Modal Card */}
      <div 
        className="relative bg-white rounded-2xl shadow-2xl w-full max-w-3xl overflow-hidden flex flex-col max-h-[90vh] text-left border border-slate-100 z-10 animate-[pageFadeIn_0.2s_ease]"
      ...
      >
        ...
        <div className="p-6 overflow-y-auto space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 bg-slate-50 p-4 rounded-2xl border border-slate-100">
            <div className="flex items-center gap-3">
              ...
              <div>
                <span className="text-[10px] text-slate-400 font-semibold block">ผู้รายงานหลัก</span>
          ...
          {/* Multi-Department Collaboration & Forwarding Badges (WangchanBERTa) */}
          {(() => {
            const multi = ticket.llm_analysis?.multi_categories || [];
            const topScores = (ticket.llm_analysis?.all_category_scores || [])
              .filter(s => s.category_name !== ticket.category_name && s.confidence >= 0.15);
            const related = multi.length > 0 ? multi : topScores;
              ...
              <div className="p-4 bg-amber-50 rounded-2xl border border-amber-200/80 space-y-2.5">
                <div className="flex items-center justify-between text-xs font-bold text-amber-900">
                  <span className="flex items-center gap-1.5">
                    ...
                    หมวดหมู่อื่นที่เกี่ยวข้อง / ส่งต่องานร่วมกัน (Multi-Label Routing):
                  </span>
                ...
                <div className="flex flex-wrap gap-2">
                  {related.map((c, i) => (
                    <div key={i} className="flex items-center gap-1.5 bg-white px-3 py-1.5 rounded-xl border border-amber-300 shadow-2xs">
                      <span className="text-xs font-bold text-amber-950">{c.category_name}</span>
                    </div>
                  ))}
                </div>
              </div>
          ...
          {/* Merged Duplicates Section (Enhanced Sub-reports Breakdown) */}
          {dups.length > 0 && (
            <div className="space-y-3 pt-2">
              <div className="flex items-center justify-between">
                <h4 className="text-xs font-bold text-indigo-700 uppercase tracking-wider flex items-center gap-1.5">
                  <Sparkles size={14} className="text-indigo-600" /> 
                  รายงานปัญหาซ้ำที่ถูกรวมอยู่ในตั๋วนี้ ({dups.length} รายการ)
                </h4>
                <span className="text-[10px] font-bold bg-indigo-100 text-indigo-800 px-2.5 py-0.5 rounded-full">
                  Zero-Click SBERT Clustered
                </span>
              </div>

              <div className="space-y-2.5 max-h-60 overflow-y-auto pr-1">
                {dups.map((dup, idx) => (
                  <div key={dup.problem_id || idx} className="p-3.5 bg-indigo-50/60 rounded-xl border border-indigo-100 text-xs space-y-2 text-left">
                    <div className="flex items-center justify-between font-bold text-slate-700 flex-wrap gap-1">
                      <span className="text-indigo-800 font-mono">#{dup.ticket_id || dup.problem_id} &bull; {dup.author_name || dup.author?.display_name || "นิสิต มพ."}</span>
                      <div className="flex items-center gap-2">
                        <span className="bg-white text-indigo-700 px-2 py-0.5 rounded-md border border-indigo-200 text-[10px] font-bold">
                          🎯 ความคล้ายคลึง: 95%
                        </span>
                        <span className="text-[11px] text-slate-400">
                          {dup.created_at ? new Date(dup.created_at).toLocaleString('th-TH', { day: '2-digit', month: 'short', hour: '2-digit', minute: '2-digit' }) : ''} น.
                        </span>
                      </div>
                    </div>
                    <p className="text-slate-700 leading-relaxed font-sans">{dup.description}</p>
                    {dup.building_name && (
                      <div className="text-[11px] text-slate-500 flex items-center gap-1">
                        <MapPin size={11} className="text-indigo-500" />
                        <span>สถานที่ตามที่ผู้แจ้งระบุ: {dup.building_name}</span>
                      </div>
                    )}
                  </div>
                ))}
        ...
        {/* Action Footer */}
        <div className="px-6 py-4 border-t border-slate-100 bg-slate-50/80 flex items-center justify-between flex-wrap gap-3">
          <div className="flex items-center gap-2">
