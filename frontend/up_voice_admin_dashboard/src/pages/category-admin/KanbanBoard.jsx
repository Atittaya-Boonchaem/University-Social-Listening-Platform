...
export default function KanbanBoard() {
  ...
  // AI Confidence Map for unmerged tickets (Strict Topic Match)
  const aiMatchMap = useMemo(() => {
    const map = {};
    tickets.forEach(ticket => {
      if (ticket.is_hidden || ticket.parent_problem_id) return;
      const t1 = (ticket.title || '').trim().toLowerCase();
      if (t1.length < 4) return;
      
      let highestScore = 0;
      let matchCount = 0;

      tickets.forEach(other => {
        if (other.problem_id === ticket.problem_id || other.is_hidden || other.parent_problem_id || other.category_id !== ticket.category_id) return;
        const t2 = (other.title || '').trim().toLowerCase();
        if (t2.length < 4) return;
        
        // Check meaningful word overlap (excluding generic common stop words)
        const stopWords = new Set(['ตึก', 'อาคาร', 'คณะ', 'และ', 'การ', 'ที่', 'มี', 'ณ', 'ใน', 'ของ', 'เป็น', 'ไม่', 'รายงาน', 'ปัญหา', 'ict']);
        const words1 = t1.split(/[\s,]+/).filter(w => w.length >= 3 && !stopWords.has(w));
        const words2 = t2.split(/[\s,]+/).filter(w => w.length >= 3 && !stopWords.has(w));
        
        let matchingWords = 0;
        words1.forEach(w => {
          if (words2.some(w2 => w2.includes(w) || w.includes(w2))) matchingWords++;
        });

        if (words1.length > 0 && matchingWords >= 2) {
          const overlapRatio = matchingWords / Math.max(words1.length, words2.length);
      ...
      if (matchCount > 0 && highestScore >= 70) {
        ...
        {COLUMNS.map((col) => {
          ...
          return (
            ...
            >
              {/* Column header */}
              {(() => {
                const totalColReports = colTickets.reduce((sum, t) => sum + 1 + tickets.filter(x => x.parent_problem_id === t.problem_id).length, 0);
                return (
                  <div className={`flex items-center justify-between px-4 py-3 rounded-xl border-2 shadow-sm ${col.headerBg}`}>
                    ...
                    <span className="px-2.5 py-1 rounded-full bg-white/90 border border-black/10 flex items-center justify-center text-xs font-black text-slate-700 shadow-sm">
                      {colTickets.length} กลุ่มงาน {totalColReports > colTickets.length ? `(${totalColReports} รายงาน)` : ''}
                    </span>
                  </div>
                );
              })()}
              ...
              <div className="flex flex-col gap-3 flex-1">
                {colTickets.length === 0 ? (
                  ...
                  colTickets.map(ticket => {
                    const dups = tickets.filter(t => t.parent_problem_id === ticket.problem_id);
                    const reportCount = 1 + dups.length;
                    return (
                      <div
                      ...
                      >
                        <TicketCard
                          ticket={{ ...ticket, reportCount, duplicates: dups }}
                          aiMatch={aiMatchMap[ticket.problem_id]}
                          onQuarantine={handleQuarantine}
                          onForward={() => setForwardTarget(ticket)}
                          onViewDetail={() => setDetailTarget({ ...ticket, reportCount, duplicates: dups })}
