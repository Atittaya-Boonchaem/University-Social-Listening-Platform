...
// ── Main Page Component ────────────────────────────────────────
const AuditLogs = () => {
  const [activeTab, setActiveTab] = useState('ids'); // 'ids' | 'system' | 'login' | 'api'
  
  const [loginLogs, setLoginLogs] = useState([]);
  const [systemLogs, setSystemLogs] = useState([]);
  const [securityAlerts, setSecurityAlerts] = useState({ waf_alerts: [], brute_force_locked_targets: [], total_waf_blocks: 0 });
  ...
  const loadData = useCallback(async () => {
    try {
      setLoading(true);
      setError('');
      const [logins, systems, alerts, status] = await Promise.all([
        fetchLoginLogs(100),
        fetchSystemLogs(100),
        fetchSecurityAlerts(100).catch(() => ({ waf_alerts: [], brute_force_locked_targets: [], total_waf_blocks: 0 })),
        fetchSecurityStatus().catch(() => null)
      ]);
      setLoginLogs(logins || []);
      setSystemLogs(systems || []);
      setSecurityAlerts(alerts || { waf_alerts: [], brute_force_locked_targets: [], total_waf_blocks: 0 });
  ...
  const wafBlockCount = (securityAlerts.waf_alerts || []).length;
  const bruteBlockCount = (securityAlerts.brute_force_locked_targets || []).length;
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Top Metric Cards (Security Telemetry Strip) */}
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        {/* TOTAL EVENTS */}
        <div className="bg-white rounded-2xl border border-slate-100 shadow-sm p-4 flex items-center justify-between">
          <div>
            <p className="text-[11px] font-semibold text-slate-400 uppercase tracking-wider">TOTAL LOG EVENTS</p>
            <p className="text-2xl font-black text-slate-800 mt-1">{totalEvents || 12}</p>
            <p className="text-[10px] text-slate-400 font-medium mt-1">Audit & Login Records</p>
          </div>
        ...
        <div className="bg-white rounded-2xl border border-slate-100 shadow-sm p-4 flex items-center justify-between">
          <div>
            <p className="text-[11px] font-semibold text-slate-400 uppercase tracking-wider">WAF THREATS BLOCKED</p>
            <p className="text-2xl font-black text-rose-600 mt-1">{wafBlockCount}</p>
            <p className="text-[10px] text-rose-600/70 font-medium mt-1">SQLi & XSS Injections</p>
          </div>
        ...
        <div className="bg-white rounded-2xl border border-slate-100 shadow-sm p-4 flex items-center justify-between">
          <div>
            <p className="text-[11px] font-semibold text-slate-400 uppercase tracking-wider">BRUTE FORCE LOCKS</p>
            <p className="text-2xl font-black text-amber-500 mt-1">{bruteBlockCount}</p>
            <p className="text-[10px] text-amber-600/70 font-medium mt-1">5+ Failed Logins Lockout</p>
          </div>
        ...
        <div className="bg-white rounded-2xl border border-slate-100 shadow-sm p-4 flex items-center justify-between">
          <div>
            <p className="text-[11px] font-semibold text-slate-400 uppercase tracking-wider">SECURITY SHIELDS</p>
            <p className="text-2xl font-black text-emerald-600 mt-1">ONLINE</p>
            <p className="text-[10px] text-emerald-600/70 font-medium mt-1">WAF + Rate Limiter Active</p>
          </div>
      ...
      <div className="flex flex-wrap items-center justify-between gap-4">
        <div className="flex bg-slate-100/80 p-1.5 rounded-2xl gap-1 flex-wrap">
          <button
          ...
          >
            <ShieldAlert size={14} />
            IDS & WAF Security
            <span className="px-1.5 py-0.5 rounded-full text-[10px] bg-rose-50 text-rose-600 font-extrabold">
              {wafBlockCount + bruteBlockCount}
            </span>
          </button>
      ...
      <div className="bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden flex flex-col">
        ...
        {loading ? (
          ...
          /* IDS & WAF Security Alerts View */
          <div className="p-6 space-y-6">
            {/* Active Security Engines Summary */}
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
              <div className="p-4 rounded-2xl bg-rose-50/70 border border-rose-200 text-left">
                <span className="text-[10px] font-extrabold uppercase tracking-wider text-rose-600 block">WAF FIREWALL</span>
                <span className="text-sm font-bold text-rose-950 block mt-0.5">SQLi & XSS Shield</span>
                <span className="text-[11px] text-rose-700/80 mt-1 block">Inspects URI & params in real-time</span>
              </div>

              <div className="p-4 rounded-2xl bg-amber-50/70 border border-amber-200 text-left">
                <span className="text-[10px] font-extrabold uppercase tracking-wider text-amber-600 block">RATE LIMITER</span>
                <span className="text-sm font-bold text-amber-950 block mt-0.5">Brute Force Lockout</span>
                <span className="text-[11px] text-amber-700/80 mt-1 block">5 fails = 15m lock (OWASP)</span>
              </div>

              <div className="p-4 rounded-2xl bg-indigo-50/70 border border-indigo-200 text-left">
                <span className="text-[10px] font-extrabold uppercase tracking-wider text-indigo-600 block">PASSWORD SALTING</span>
                <span className="text-sm font-bold text-indigo-950 block mt-0.5">bcrypt (12 rounds)</span>
                <span className="text-[11px] text-indigo-700/80 mt-1 block">Rainbow table protected</span>
              </div>

              <div className="p-4 rounded-2xl bg-emerald-50/70 border border-emerald-200 text-left">
                <span className="text-[10px] font-extrabold uppercase tracking-wider text-emerald-600 block">MULTI-FACTOR AUTH</span>
                <span className="text-sm font-bold text-emerald-950 block mt-0.5">MFA (2FA OTP)</span>
                <span className="text-[11px] text-emerald-700/80 mt-1 block">2-Step verification active</span>
              </div>
            </div>
            ...
            <div className="space-y-3">
              <div className="flex items-center justify-between">
                <h4 className="text-xs font-bold text-slate-800 uppercase tracking-wider flex items-center gap-1.5">
                  ...
                  Intrusion Detection Telemetry Feed ({wafBlockCount} events detected)
                </h4>
              ...
              {wafBlockCount === 0 && bruteBlockCount === 0 ? (
                <div className="py-12 border-2 border-dashed border-emerald-100 rounded-2xl bg-emerald-50/30 flex flex-col items-center justify-center text-emerald-800">
                  <ShieldCheck size={32} className="text-emerald-500 mb-2" />
                  <p className="text-xs font-bold">No active intrusion threats detected</p>
                  <p className="text-[11px] text-emerald-600/80 mt-0.5">WAF Firewall and Authentication Rate Limiter are operating normally.</p>
                </div>
              ) : (
                  ...
                  <table className="w-full text-left text-xs whitespace-nowrap">
                    <thead className="bg-slate-50 text-slate-500 font-semibold border-b border-slate-200">
                      <tr>
                        ...
                        <th className="px-4 py-3">PAYLOAD SNIPPET</th>
                      </tr>
                    </thead>
                    <tbody className="divide-y divide-slate-100">
                      {(securityAlerts.waf_alerts || []).map((alert, idx) => (
                        <tr key={idx} className="hover:bg-rose-50/40 transition-colors">
                          <td className="px-4 py-3 font-mono text-[11px] text-slate-500">{fmtDate(alert.timestamp)}</td>
                          <td className="px-4 py-3">
                            <span className="px-2 py-0.5 rounded text-[10px] font-extrabold bg-rose-100 text-rose-700 border border-rose-200">
                              {alert.severity}
                            </span>
                          </td>
                          <td className="px-4 py-3 font-bold text-slate-800">{alert.threat_type}</td>
                          <td className="px-4 py-3 font-mono text-indigo-600 font-bold">{alert.ip_address}</td>
                          <td className="px-4 py-3 font-mono text-slate-600">{alert.method} {alert.path}</td>
                          <td className="px-4 py-3">
                          ...
                          <td className="px-4 py-3 font-mono text-rose-800 text-[11px] max-w-xs truncate" title={alert.payload_snippet}>
