...
export default function GlobalDashboard() {
  ...
  const visibleProblems = selectedCategory === 'ALL' ? allProblems : allProblems.filter(p => p.category_name === selectedCategory);
  const byStatus = selectedCategory === 'ALL' ? (analytics?.by_status ?? {}) : visibleProblems.reduce((acc, p) => { const key = p.status_name || 'UNKNOWN'; acc[key] = (acc[key] || 0) + 1; return acc; }, {});
  const byCategory = selectedCategory === 'ALL' ? (analytics?.by_category ?? []) : [{ category_name: selectedCategory, count: visibleProblems.length }];
  const total = selectedCategory === 'ALL' ? (analytics?.total ?? 0) : visibleProblems.length;
  const geoPoints = selectedCategory === 'ALL' ? geoPointsState : geoPointsState.filter(p => p.category_name === selectedCategory);
  ...
  return (
    <div className="p-6 md:p-8 space-y-6 max-w-7xl mx-auto font-sans">
      ...
      <div className="flex items-center justify-between gap-3">
        ...
        <button
          onClick={loadAll}
          className="ml-auto flex items-center gap-2 px-4 py-2 bg-white border border-slate-200 rounded-xl text-sm font-semibold text-slate-600 hover:border-indigo-300 hover:text-indigo-600 transition-all shadow-sm"
        >
          <RefreshCw size={14} />
          รีเฟรชข้อมูล
        </button>
        <select value={selectedCategory} onChange={(e) => setSelectedCategory(e.target.value)} className="px-3 py-2 bg-white border border-slate-200 rounded-xl text-sm font-semibold text-slate-600 shadow-sm" aria-label="เลือก Dashboard ตามหมวดหมู่">
          <option value="ALL">ภาพรวมทุกหมวดหมู่</option>
