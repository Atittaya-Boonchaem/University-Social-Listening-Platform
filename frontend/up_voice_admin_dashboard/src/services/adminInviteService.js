...
/**
 * Resend an invite (generate fresh token & reset expiration).
 * @param {number} inviteId - The ID of the invite
 * @param {number} [expirationDays=7] - Duration in days
 */
export async function resendInvite(inviteId, expirationDays = 7) {
  const res = await api.post(`/users/invites/${inviteId}/resend`, { expiration_days: expirationDays });
  return res.data;
}

/**
 * Extend an invite's expiration date.
 * @param {number} inviteId - The ID of the invite
 * @param {number} [days=7] - Days to extend by
 */
export async function extendInvite(inviteId, days = 7) {
  const res = await api.patch(`/users/invites/${inviteId}/extend`, { days });
  return res.data;
}
